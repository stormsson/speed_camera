...
2. **Given** a video where a car crosses both left and right coordinates, **When** the system processes the video, **Then** it correctly identifies the frame when the car's rightmost bounding box edge first transitions from being less than the left coordinate to being greater than or equal to it, and the frame when it transitions from being less than the right coordinate to being greater than or equal to it
...
7. **Given** a video being processed with the `--debug` parameter enabled, **When** a crossing event is detected (left or right coordinate), **Then** the system generates a PNG file named `crossing_[frame_number].png` in the project folder showing the frame with bounding box, coordinate lines, and detection criteria text
...
- **FR-004**: System MUST identify the frame when a car first crosses the left_coordinate (crossing occurs on the first frame where the rightmost edge of the car's bounding box transitions from being less than the left_coordinate to being greater than or equal to the left_coordinate)
- **FR-005**: System MUST identify the frame when the same car crosses the right_coordinate (crossing occurs on the first frame where the rightmost edge of the car's bounding box transitions from being less than the right_coordinate to being greater than or equal to the right_coordinate)
...
- **FR-015**: System MUST support an optional `--debug` parameter that, when enabled, generates a PNG image file named `crossing_[frame_number].png` in the project folder for each crossing event detected. Each debug image MUST display: (1) the car's bounding box, (2) vertical lines overlaying the left_coordinate and right_coordinate measurement lines, and (3) a text string showing the crossing detection criteria (e.g., "bounding box X: [coord] - left line coord: [coord]" or "bounding box X: [coord] - right line coord: [coord]")
...
- Q: When exactly should a crossing event be detected - at the moment of transition or when the condition is already true? → A: Crossing must be detected on the first frame where the rightmost edge of the bounding box transitions from being less than the coordinate to being greater than or equal to the coordinate. This requires checking the previous frame state to detect the transition, ensuring the crossing is recorded at the exact moment it occurs, not after the car has already passed the line
